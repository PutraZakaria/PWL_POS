            ...
            @else
                <form method="POST" action="{{ url('/stok/' . $stok->stok_id) }}" class="form-horizontal">
                        ...
                        <div class="col-11">
                            <select class="form-control" id="barang_id" name="barang_id" required>
                                ...
                                @foreach ($barang as $item)
                                    <option value="{{ $item->barang_id }}" @if ($item->barang_id == $stok->barang_id)
                                        selected @endif>{{ $item->barang_nama }}</option>
                                @endforeach
                            </select>
                            @error('barang_id')
                        ...
                        <div class="col-11">
                            <select class="form-control" id="user_id" name="user_id" required>
                                ...
                                @foreach ($user as $item)
                                    <option value="{{ $item->user_id }}" @if ($item->user_id == $stok->user_id)
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-11">
                            <button type="submit" class="btn btn-primary btn-sm" href="{{ url('stok/' . $stok->stok_id) }}">Simpan</button>
